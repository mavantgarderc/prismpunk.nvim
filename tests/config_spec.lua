...
describe("config.themes", function()
  it("has empty array as default", function()
    config.setup({})
    assert.is_table(config.defaults.themes)
    assert.equal(0, #config.defaults.themes)
  end)

  it("accepts themes array in setup", function()
    local result = config.setup({
      themes = { "dc/superman", "kanagawa" },
    })
    assert.is_table(result.themes)
    assert.equal(2, #result.themes)
  end)
end)

describe("config.is_theme_allowed", function()
  before_each(function()
    config.setup({})
  end)

  it("allows all themes when config.themes is empty", function()
    assert.is_true(config.is_theme_allowed("dc/superman"))
    assert.is_true(config.is_theme_allowed("kanagawa/paper-edo"))
    assert.is_true(config.is_theme_allowed("tmnt/leonardo"))
  end)

  it("allows explicitly listed themes", function()
    config.setup({ themes = { "dc/superman", "kanagawa" } })
    assert.is_true(config.is_theme_allowed("dc/superman"))
    assert.is_true(config.is_theme_allowed("kanagawa/paper-edo"))
  end)

  it("rejects themes not in config", function()
    config.setup({ themes = { "dc/superman" } })
    assert.is_false(config.is_theme_allowed("tmnt/leonardo"))
    assert.is_false(config.is_theme_allowed("kanagawa/paper-edo"))
  end)

  it("allows entire universe when universe is listed", function()
    config.setup({ themes = { "dc" } })
    assert.is_true(config.is_theme_allowed("dc/superman"))
    assert.is_true(config.is_theme_allowed("dc/batman"))
    assert.is_true(config.is_theme_allowed("dc/lantern-corps/green"))
  end)
end)

describe("config.get_allowed_themes", function()
  it("returns empty table by default", function()
    config.setup({})
    local themes = config.get_allowed_themes()
    assert.is_table(themes)
    assert.equal(0, #themes)
  end)
